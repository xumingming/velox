...
// Match input with a kRelaxedFixed pattern.
bool matchRelaxedFixedPattern(
    StringView input,
    const std::vector<SubPatternMetadata>& subPatterns,
    size_t length) {
...
// here), and search the first kLiteralString string in input to find the
// position to match.
bool matchRelaxedSubstringPattern(
  ...
  while (indexInString <= end) {
    ...
    // Check the indexInString is within current input boundary.
    // Why we need this check? In Velox StringView's data is stored together
    // in a memory region with no terminating '\0', we need to check the char*
    // boundary manually.
    if (indexInString > end) {
      return false;
    }
...
PatternMetadata determinePatternKind(
  ...
  const auto numSubPatterns = subPatterns.size();
  ...
  if (numSubPatterns == 1) {
    if (firstSubPattern.kind ==
      ...
      return PatternMetadata::fixed(std::move(subPatterns));
    }
    ...
    if (literalStats.count == 0) {
      ...
      if (singleCharWildcardStats.count > 0 &&
        ...
        auto literalOrSingleWildcardPatternsMaker =
            [&](std::vector<SubPatternMetadata>& result) {
              result.reserve(
        ...
        if (firstOfLiteralOrSingleWildcard > 0 &&
          ...
          if (!containsAnyWildcardBetween) {
            std::vector<SubPatternMetadata> result;
            literalOrSingleWildcardPatternsMaker(result);
            return PatternMetadata::relaxedSubstring(std::move(result));
          }
        } else if (
            lastOfLiteralOrSingleWildcard < anyCharsWildcardStats.lastIndex) {
          std::vector<SubPatternMetadata> result;
          literalOrSingleWildcardPatternsMaker(result);
          return PatternMetadata::relaxedPrefix(std::move(result));
        } else if (
            firstOfLiteralOrSingleWildcard > anyCharsWildcardStats.lastIndex) {
          std::vector<SubPatternMetadata> result;
          literalOrSingleWildcardPatternsMaker(result);
          return PatternMetadata::relaxedSuffix(std::move(result));
        }
      }
    }
  }
