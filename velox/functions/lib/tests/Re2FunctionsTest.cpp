...
TEST_F(Re2FunctionsTest, likeRelaxedSubstringPatternManyStrings) {
  auto makeInput =
      [&](vector_size_t vectorSize, bool padAtHead, bool padAtTail) {
        return vectorMaker_.flatVector<std::string>(vectorSize, [&](auto row) {
  ...
  VectorPtr input = makeInput(20, true, true);
  VectorPtr result;

  result = evaluate(R"(like(c0, '%_\_b_c%', '\'))", makeRowVector({input}));
  auto expected =
      makeFlatVector<bool>(20, [](auto row) { return row % 2 == 0; });
